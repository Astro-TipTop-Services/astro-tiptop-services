...
This page gathers all the resources available to learn and use the TipTop Asterism Selection module.

Whether you simply want to reproduce one of the existing examples or understand the complete workflow, you will find both the official examples distributed with TipTop and a detailed step-by-step tutorial.

## 📚 Available examples

Several complete examples are distributed with the TipTop repository and illustrate the different guide-star selection workflows. <br/>
These notebooks provide ready-to-run configurations.

| Example | AO system | Purpose |
|----------|-----------|---------|
| [`TIPTOP-AST-EXAMPLE-ERIS.ipynb`](https://github.com/astro-tiptop/TIPTOP/blob/main/examples/TIPTOP-AST-EXAMPLE-ERIS.ipynb) | ERIS LGS | Low-order guide-star selection |
| [`TIPTOP-AST-EXAMPLE-ERIS-HO.ipynb`](https://github.com/astro-tiptop/TIPTOP/blob/main/examples/TIPTOP-AST-EXAMPLE-ERIS-HO.ipynb) | ERIS NGS | High-order guide-star selection (`hoAsterismSelection`) |
| [`TIPTOP-AST-EXAMPLE-MAVIS.ipynb`](https://github.com/astro-tiptop/TIPTOP/blob/main/examples/TIPTOP-AST-EXAMPLE-MAVIS.ipynb) | MAVIS | Three-star asterism selection |
| [`ERIS_Asterism.py`](https://github.com/astro-tiptop/TIPTOP/blob/main/examples/ERIS_Asterism.py) | Random fields | Heuristic-model training & benchmarking |

👉 All examples are available in the [examples directory](https://github.com/astro-tiptop/TIPTOP/tree/main/examples)
of the TipTop repository.
...
<p align="justify">
The following notebook illustrates the complete Asterism Selection workflow, from configuring an input file to identifying the optimal guide-star configuration and running the final AO simulation.
</p>

**Download the complete notebook:**
<a href={`${useBaseUrl('/downloads/TIPTOP_Asterism_Selection_Tutorial.ipynb')}`} download="TIPTOP_Asterism_Selection_Tutorial.ipynb">**here**<span style={{ fontSize: '1.5em' }}>📥</span></a>.

### This tutorial covers

1. Configure an `.ini` file with an `[ASTERISM_SELECTION]` block.
2. Run `asterismSelection(...)`.
3. Analyse and rank candidate asterisms.
4. Export results to JSON.
5. Re-run the best configuration with `overallSimulation(...)`.
6. Reload previous simulations.
7. Train and use a heuristic model (optional).
...
- `astro-tiptop` version 1.5.0 or higher
...
FWHM (and sometimes EE) can be an array per asterism (several science points).
...
Fields: 10<br/>
Field 000 → best asterism (global index=2) [penalty=218.2, SR=0.5014, FWHM=48.49]<br/>
Field 001 → best asterism (global index=7) [penalty=407.1, SR=0.09045, FWHM=76.38]<br/>
Field 002 → best asterism (global index=12) [penalty=253.1, SR=0.3951, FWHM=53.25]<br/>
Field 003 → best asterism (global index=16) [penalty=1079, SR=4.628e-08, FWHM=187.9]<br/>
Field 004 → best asterism (global index=18) [penalty=288.5, SR=0.299, FWHM=58.33]<br/>
Field 005 → best asterism (global index=23) [penalty=439, SR=0.06116, FWHM=81.43]<br/>
Field 006 → best asterism (global index=26) [penalty=482.5, SR=0.0342, FWHM=88.39]<br/>
Field 007 → best asterism (global index=29) [penalty=259.9, SR=0.3754, FWHM=54.22]<br/>
Field 008 → best asterism (global index=33) [penalty=602.6, SR=0.005163, FWHM=108]<br/>
Field 009 → best asterism (global index=36) [penalty=448.7, SR=0.05399, FWHM=82.97]<br/>
...
<p style={{ fontFamily: "Consolas" ,  fontSize: '0.8rem',  backgroundColor: '#D1FAE5' }}>
✅ Reusing step-2 metrics (in memory).. <br/>
📝 Wrote INI (minimal): C:\Users\user\AppData\Local\Temp\tiptop_best_q8dtcqyv\ERISast10_best_ast.ini <br/>
✅ overallSimulation done.<br/>
...
## 5) Reload previously computed metrics
...
## 6) (Optional) Train & test a heuristic model
...
- Trains the model and, if `doTest=True`, reports accuracy and ranking quality.
...
```python
[ASTERISM_SELECTION]
heuristicModel = 'ERISastRandom_hmodel'  
```
