...
If you want to access the **output PSFs and metrics as Python objects** (e.g., for easier integration in your software) and **avoid writing and opening** the result `FITS` file
you can: <br/>
✔️ Use the path with `BaseSimulation`: instantiate → `doOverallSimulation()` → `computeMetrics()`; and, if needed,
convert to NumPy with `cpuArray`. See the [**`baseSimulation`**](/docs/orion/howtosetuplaunchfile#baseSimulation) documentation below. <br/>
...
#%% (optional) Ensure NumPy arrays (avoid NumPy/CuPy mixing)
if getattr(sim, "psfOL", None) is not None:
    sim.psfOL.sampling = cpuArray(sim.psfOL.sampling)
if getattr(sim, "psfDL", None) is not None:
    sim.psfDL.sampling = cpuArray(sim.psfDL.sampling)
if getattr(sim, "psf1d_data", None) is not None:
    sim.psf1d_data = cpuArray(sim.psf1d_data)
if hasattr(sim, 'PSD') and sim.PSD is not None:
    sim.PSD = cpuArray(sim.PSD)

#%% 4) Use results directly, in memory
psf_ao   = sim.cubeResultsArray   # AO-corrected PSF
psf_ol   = sim.psfOL.sampling     # Seeing-limited PSF
psf_dl   = sim.psfDL.sampling     # Diffraction-limited PSF
profiles = sim.psf1d_data         # PSFs profiles
psd      = sim.PSD                # PSD

#%% Extract useful parameter 
wvl     = sim.wvl       # Wavelength in m
pix_mas = sim.psInMas   # Pixel scale in milliarcseconds
sr      = sim.sr        # Strehl ratio
fwhm    = sim.fwhm      # FWHM in milliarcsecond
...
# If you eventually want a FITS later:
# sim.saveResults()
...
```python
sim = baseSimulation(path2param, parametersFile, outputDir, outputFile, doConvolve, doPlot, 
                     addSrAndFwhm, verbose, getHoErrorBreakDown, savePSDs, ensquaredEnergy, eeRadiusInMas)
```
...
- **outputDir** (_str_, required) — Path to the folder in which to write the output (used if `sim.saveResults()` called).
- **outputFile** (str, required) — Prefix for output files (`.fits`, `.json`) when saving.
...
- **addSrAndFwhm** (_bool_, optional, default: `True`) — The function will add in the header of the fits file SR, FWHM, and EE within eeRadiusInMas for each PSF.
...
- **eeRadiusInMas** (_float_, optional, default: 50) – Radius used for the computation of the encirlced energy 
 (if `ensquaredEnergy` is selected, this is half the side of the square.)
</details> 
...
<details>
  <summary><strong>Parameters:</strong></summary>
    ...
    - **returnRes** (_bool_) – optional – default: `False`, If `True`, the function will return the High Order residual and the Low Order residual (if LO) in nm RMS.
    ...
    - **getHoErrorBreakDown** (_bool_) – optional – default: False, If you want HO error breakdown set this to `True` (**verbose** must also to be set to `True`).
...
⚠️ **Note:** If `returnMetrics=True` or `returnRes=True`, no `FITS` file is written. Use both flags as `False` to save the `FITS`.
