...
These configurable parameters can help you to produce more realistic PSFs, test “what-if” error budgets, or reproduce measured PSFs for specific analyses. <br/>
...
<i>“[Error terms coverage](/docs/general/error_breakdown)”</i> to avoid adding errors that are already taken into account.
...
| `extraErrorMax` | No | `float` | _Default:_ `0`, Maximum spatial frequency <code>[m<sup>−1</sup>]</code> for which PSD associated with `extraErrorNm` is `> 0`. <br/> _Note_: `0` means maximum frequency is the one present in the spatial frequency array of the PSDs.|
...
TipTop’s pipeline (in [<code>tiptop.baseSimulation.finalConvolution()</code>](https://github.com/astro-tiptop/TIPTOP/blob/d5cceb2d215e81004218b67bca10b6ef87dc8b92/tiptop/baseSimulation.py#L530C9-L530C25)).
...
<div style={{ textAlign: "justify" }}> 
<ul> <li><code>extraErrorNm</code> – total RMS amplitude (in _nm_) of the additional error</li> 
<li><code>extraErrorExp</code> – exponent of the power-law spectrum (default –2)</li> 
<li><code>extraErrorMin</code> / <code>extraErrorMax</code> – optional lower/upper spatial-frequency limits (in _m<sup>−1</sup>_).</li> </ul> 
</div>
...
In the "MUSE PSF-fitting" example shown below (for a science wavelength of 500<i>nm</i>), 
...
Both contributions are combined into an on-axis OPD map (in _nm_) inside the
...
The values are given in <i>nm RMS</i> and follow the standard <b>Noll index</b> ordering (Z2 = tilt, Z3 = tilt, Z4 = defocus, Z5–Z6 = astigmatism, …).
...
<li><b>PSF shape modification</b> — Zernike modes introduce directional features (e.g. astigmatism, coma), while arbitrary OPD maps can create more complex patterns.</li> 
...
✏️ **Note:** This script uses the high-level `overallSimulation` function and writes a temporary `.ini` file for each run. If you need in-memory parameter sweeps without disk I/O (e.g. in a service context), see the [parameter sweep example using `baseSimulation` and `my_data_map`](/docs/orion/howtosetuplaunchfile#param-sweep) instead.
</div>
